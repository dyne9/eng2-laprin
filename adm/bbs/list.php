...
if (!$is_search_bbs) {
    ...
    for ($k=0; $k<$board_notice_count; $k++) {
        ...
        $list[$i] = get_list($row, $board, $board_skin_url, G5_IS_MOBILE ? $board['bo_mobile_subject_len'] : $board['bo_subject_len'], $bbsPath=G5_ADM_BBS_URL);
...
$next_part_href = '';
if ($is_search_bbs) {
    $list_href = '/adm/bbs/board.php?bo_table='.$bo_table;
    ...
    if ($next_spt < 0) {
        $qstr1 = preg_replace($patterns, '', $qstr);
        $next_part_href = '/adm/bbs/board.php?bo_table='.$bo_table.$qstr1.'&amp;spt='.$next_spt.'&amp;page=1';
...
if ($member['mb_level'] >= $board['bo_write_level']) {
    $write_href = '/adm/bbs/write.php?bo_table='.$bo_table;
}
